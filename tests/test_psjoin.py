...
def run_psjoin(
    ...
    if file_type == ".pdf":
        compare_bytes(
            ...
            datafiles / Path("output").with_suffix(file_type),
            expected_file.with_suffix(file_type),
        )
    else:
        compare_strings(
            capsysbinary.readouterr().out.decode("utf-8"),
            datafiles / Path("output").with_suffix(file_type),
            expected_file.with_suffix(file_type),
        )
...
def test_psjoin_1_2(
    capsysbinary: CaptureFixture[bytes], files: Tuple[Path, ...], file_type: str
) -> None:
...
def test_psjoin_1_2_even(
    capsysbinary: CaptureFixture[bytes], files: Tuple[Path, ...], file_type: str
) -> None:
    datafiles, test_file, expected_file = files
    run_psjoin(
        [
            str(test_file.with_suffix(file_type)),
            str(test_file.with_suffix(file_type)),
            "--even",
        ],
        file_type,
...
def test_psjoin_1_2_nostrip(
    capsysbinary: CaptureFixture[bytes], files: Tuple[Path, ...]
) -> None:
    datafiles, test_file, expected_file = files
    run_psjoin(
