...
from psutils.pdftopdf import main as pdftopdf
...
class ToggleAction(argparse.Action):
    def __init__(self, option_strings: List[str], dest: str, nargs: Optional[str]=None, default: bool=False, **kwargs: Any) -> None:
...
# Command-line arguments
    parser = argparse.ArgumentParser(
        description='Put multiple pages of a PDF document on to one page.',
        formatter_class=HelpFormatter,
        usage='%(prog)s [OPTION...] -NUMBER [INFILE [OUTFILE]]',
        add_help=False,
        epilog='''
pdfnup aborts with an error if it cannot arrange the input pages so as to
...
''',
    ...
    parser.add_argument('-m', '--margin', metavar='DIMENSION',
                        type=parsedimen, default=0,
                        help='''\
...
as the original page margins will be shrunk''')
    parser.add_argument('-b', '--border', metavar='DIMENSION',
                        type=parsedimen, default=0,
...
default is no line]''')
    parser.add_argument('-l', '--rotatedleft', action=ToggleAction,
                        help='input pages are rotated left 90 degrees')
    parser.add_argument('-r', '--rotatedright', action=ToggleAction,
                        help='input pages are rotated right 90 degrees')
    parser.add_argument('-f', '--flip', action=ToggleAction,
                        help="swap output pages' width and height")
    parser.add_argument('-c', '--transpose', action=ToggleAction,
                        help='swap columns and rows (column-major order)')
    parser.add_argument('-t', '--tolerance', metavar='NUMBER',
    ...
    parser.add_argument('nup', metavar='-NUMBER', type=parsenup,
...
def main(argv: List[str]=sys.argv[1:]) -> None: # pylint: disable=dangerous-default-value
    ...
    # Command line for pdftopdf
    cmd = []

    # Tell pdftopdf input paper size if it differs from output paper size
    ...
    def reduce_waste(hor: float, ver: float, iwid: float, ihgt: float, rot: float) -> None:
    ...
    if rotate:
        topbottom, leftright, rowmajor, iwidth, iheight = not leftright, topbottom, not rowmajor, iheight, iwidth

    # Calculate page scale, allowing for internal borders
    scale = min((pphgt - 2 * args.border * vert) / (iheight * vert), \
                (ppwid - 2 * args.border * horiz) / (iwidth * horiz))

    # Page centring shifts
    hshift, vshift = (ppwid / horiz - iwidth * scale) / 2, (pphgt / vert - iheight * scale) / 2
    ...
    for page in range(args.nup):
        across, up = (page % horiz, page // horiz) if rowmajor else (page // vert, page % vert)
        if not leftright:
