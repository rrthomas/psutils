...
from psutils import HelpFormatter, die, setup_input_and_output, simple_warning
from psutils.pdftopdf import main as pdftopdf

def get_parser() -> argparse.ArgumentParser:
    # Command-line arguments
    parser = argparse.ArgumentParser(
        description='Rearrange pages in a PDF document into signatures.',
...
def main(argv: List[str]=sys.argv[1:]) -> None: # pylint: disable=dangerous-default-value
    ...
    infile, _ = setup_input_and_output(args.infile, args.outfile, True, True)

    # Get number of pages
    pdfinfo = PdfReader(infile)
    input_pages = len(pdfinfo.pages)
